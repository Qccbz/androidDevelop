...
import q.baselibrary.net.HttpConstants;
import q.baselibrary.net.HttpRequest;
import q.baselibrary.net.HttpResponse;
...
public class QHttp {
     ...
     * @param httpUrl
     * @return the response of the url, if null represents http error
     * @see QHttp#httpPost(HttpRequest)
     */
    ...
    public static String getUrlWithParas(String url, Map<String, String> parasMap) {
        StringBuilder urlWithParas = new StringBuilder(QString.isEmpty(url) ? "" : url);
    ...
    public static String joinParasWithEncodedValue(Map<String, String> parasMap) {
        ...
        if (parasMap != null && parasMap.size() > 0) {
            ...
            try {
                while (ite.hasNext()) {
                    Map.Entry<String, String> entry = (Map.Entry<String, String>) ite.next();
                    paras.append(entry.getKey()).append(EQUAL_SIGN).append(QString.utf8Encode(entry.getValue()));
                    if (ite.hasNext()) {
                        paras.append(PARAMETERS_SEPARATOR);
                    }
                }
    ...
    public static void setURLConnection(Map<String, String> requestProperties, HttpURLConnection urlConnection) {
        if (QMap.isEmpty(requestProperties) || urlConnection == null) {
            return;
        }

        for (Map.Entry<String, String> entry : requestProperties.entrySet()) {
            if (!QString.isEmpty(entry.getKey())) {
